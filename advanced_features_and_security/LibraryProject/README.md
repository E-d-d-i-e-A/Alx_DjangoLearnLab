...
### Permission Format
Django permissions follow the format: `app_label.permission_codename`
...
3. Click **Add Group**
4. Name: `Viewers`
5. Select permissions from available list
6. Move to "Chosen permissions"
7. Save
...
4. Select group(s) from "Available groups"
5. Move to "Chosen groups"
6. Save

## Security Features

1. **Custom User Model**: Extensible authentication system
2. **Permission-based Access**: Fine-grained control over operations
3. **Group-based Roles**: Easy management of user permissions
4. **Exception Handling**: `raise_exception=True` returns 403 Forbidden for unauthorized access
5. **Decorator Protection**: All sensitive views are protected

## API Endpoints

| URL | Method | Permission Required | Description |
|-----|--------|-------------------|-------------|
| `/books/` | GET | `can_view` | List all books |
| `/books/create/` | GET/POST | `can_create` | Create new book |
| `/books/<id>/edit/` | GET/POST | `can_edit` | Edit existing book |
| `/books/<id>/delete/` | GET/POST | `can_delete` | Delete book |

## Troubleshooting

### Permission Not Found Error
**Solution**: Run migrations to create permissions
```bash
python manage.py migrate
```
...
- User is not in appropriate group
- User account is not active

**Solution**:
1. Check user's group membership in admin
2. Verify group has correct permissions
3. Ensure user's `is_active` is True

### Permission Exists But User Still Denied
**Solution**:
1. Clear sessions: `python manage.py clearsessions`
2. Have user logout and login again
3. Verify in admin: User → Permissions section
